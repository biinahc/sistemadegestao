...
export default function Produtos() {
    ...
    const saveProduct = async () => {
        ...
        if (product.nome && product.quantidade && product.quantidade_minima && product.lt_kl_unid && product.marca && product.tipo_producto) {
            try {
                if (product.id) {
                    await axios.put('http://localhost:8080/produtos/update/' + product.id, product);
                    toast.current.show({ severity: 'success', summary: 'Sucesso', detail: 'Item Atualizado', life: 3000 });
                } else {
                    await axios.post('http://localhost:8080/produtos/create', product);
                ...
                console.error(error);
                toast.current.show({ severity: 'error', summary: 'Erro', detail: error.response.data.message || 'Falha ao salvar produto.', life: 3000 });
            }
        } else {
            toast.current.show({ severity: 'error', summary: 'Erro', detail: 'Campo em branco', life: 3000 });
        }
    };
    ...
    const deleteProduct = async () => {
        try {
            await axios.delete('http://localhost:8080/produtos/' + product.id);
            ...
            console.error(error);
            toast.current.show({ severity: 'error', summary: 'Erro', detail: 'Falha ao excluir item.', life: 3000 });
        }
    };

    const deleteSelectedProducts = async () => {
        let ids = selectedProducts.map(item => item.id);
        try {
            await axios.delete('http://localhost:8080/items/produtos', { data: { ids: ids } });
            toast.current.show({ severity: 'success', summary: 'Sucesso', detail: 'Itens excluídos', life: 3000 });
            setDeleteProductsDialog(false);
            setSelectedProducts(null);
            fetchProducts(); // Atualiza a lista de produtos
        } catch (error) {
            console.error(error);
            toast.current.show({ severity: 'error', summary: 'Erro', detail: 'Falha ao excluir itens selecionados.', life: 3000 });
        }
    };
    ...
    const centerToolbarTemplate = () => {
        return <h4 className="display-7">Produtos <i className="fas fa-shopping-cart"></i></h4>;
    };
    ...
    return (
        <div>
            ...
            <Dialog visible={productDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Produto" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                <div className="field">
                    <label htmlFor="tipo_producto" className="font-bold">Categoria de Produto:</label>
                    <select id="tipo_producto" className="form-select" value={product.tipo_producto} onChange={(e) => onInputChange(e, 'tipo_producto')} required>
                        <option value=""></option>
                ...
                <div className="field">
                    ...
                    {product.id && <small className="p-error">Aviso: Alterar o nome pode afetar a integridade dos dados existentes!</small>}
                </div>
                <div className="field">
                    <label htmlFor="quantidade" className="font-bold">Quantidade em estoque:</label>
                    {product.id ?
                        <InputText disabled id="quantidade" value={product.quantidade} />
                        :
                        <InputText type="number" id="quantidade" value={product.quantidade} min={0} onChange={(e) => onInputNumberChange(e, 'quantidade')} required rows={2} cols={20} className={classNames({ 'p-invalid': submitted && !product.quantidade })} />
                    }
                    {submitted && !product.quantidade && <small className="p-error">Campo é obrigatório!</small>}
                </div>
                <div className="field">
                    <label htmlFor="quantidade_minima" className="font-bold">Quantidade para gerar alerta de baixo estoque:</label>
                    <InputText type="number" id="quantidade_minima" value={product.quantidade_minima} min={0} onChange={(e) => onInputNumberChange(e, 'quantidade_minima')} required rows={2} cols={20} className={classNames({ 'p-invalid': submitted && !product.quantidade_minima })} />
                    {submitted && !product.quantidade_minima && <small className="p-error">Campo é obrigatório!</small>}
                </div>
                <div className="field">
                    <label htmlFor="lt_kl_unid" className="font-bold">Tipo de Unidade:</label>
                ...
                <div className="field">
                    <label htmlFor="marca" className="font-bold">Marca do Produto:</label>
                    ...
                    {submitted && !product.marca && <small className="p-error">Campo é obrigatório!</small>}
                    {product.id && <small className="p-error">Aviso: Alterar a marca pode afetar a integridade dos dados existentes!</small>}
                </div>
            </Dialog>

            <Dialog visible={saidaDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Saída de Produto" modal className="p-fluid" footer={saidatDialogFooter} onHide={hideDialog}>
                <div className="field">
                    <label htmlFor="tipo_producto" className="font-bold">Categoria de Produto:</label>
                    <select disabled id="tipo_producto" className="form-select" value={product.tipo_producto} onChange={(e) => onInputChange(e, 'tipo_producto')}>
                        {categorias.map(option => (
                            <option key={option.nome} value={option.nome}>{option.nome}</option>
                        ))}
                    </select>
                </div>
                <div className="field">
                    <label htmlFor="nome" className="font-bold">Nome:</label>
                    <InputText disabled id="nome" value={product.nome} onChange={(e) => onInputChange(e, 'nome')} required />
                </div>
                <div className="field">
                    <label htmlFor="marca" className="font-bold">Marca do produto:</label>
                ...
                <div className="field">
                    <label htmlFor="status" className="font-bold">Quantidade atual no estoque:</label>
                    <InputText type="number" id="status" value={product.status} onChange={(e) => onInputChange(e, 'status')} required rows={2} cols={20} />
                </div>
            ...
            <Dialog visible={deleteProductDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Confirmar" modal footer={deleteProductDialogFooter} onHide={hideDeleteProductDialog}>
                <div className="confirmation-content">
                    ...
                    {product && (
                        <span>Tem certeza de que quer excluir o Item: <b>{product.nome}</b>?</span>
                    )}
                </div>
            </Dialog>
